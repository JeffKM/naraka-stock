...
손님들이 한 달간(2026-08-01 ~ 08-30, 월·화 휴장) 가상 화폐로 요괴 테마 주식 8종을 거래하고, 최종 총자산 순위 4명(잠정)이 상품을 받는다.
...
2. **가격은 사전 생성 경로** — 매일 22:00 배치가 익일 84틱(15:00~22:00, 5분 간격)을 전부 생성해 `daily_ticks`에 저장. 장중에는 읽기만 한다. 현재가 = 현재 시각의 틱 인덱스 값. 휴장일(월·화)엔 조건부 처리 — PRD §4.1.
3. **자산은 정수(원)** — 부동소수점 금지.
4. **프론트 보간 연출은 표시용** — 틱 사이 가격 애니메이션은 눈속임일 뿐, 체결가는 항상 서버 틱 값.

## 게임 핵심 규칙 (요약)

- 장 시간: 개장일(수~일) 15:00~22:00 KST, 월·화 휴장 / 상하한: 직전 개장일 종가 ±30% / 시장가 주문만 / 매도 수수료 0.3%
