...
//   단, 단서는 불완전하다(pickVolumeTells, VOL_TELL_ACC=0.8): 헤드페이크 20%는 정상 거래량으로
//   위장하고 진짜 20%는 조용해, "오르는데 얇으면 헤드페이크"가 확실한 규칙이 아니라 ~80% 경향이
//   된다 → 거래량만 보는 판단을 배신해 "톤+시세브레이크+거래량" 종합(다층 판단)을 강제한다. 하네스 검증
...
const STOCK_EARLY_VOL_TELL_ACC = 0.8; // 거래량 단서 정확도(하네스): 진짜 80% 거래량 실림 / 헤드페이크 80% 얇음
...
export interface VolumeTells {
  quietReals: Set<string>; // 진짜인데 거래량 조용(20%) — 거래량 확인이 진짜를 놓치게
  loudHeadfakes: Set<string>; // 헤드페이크인데 거래량 정상(20%) — 거래량 확인도 함정에 빠지게
}

// 거래량 단서를 "불완전"하게 만드는 예외 집합 (Phase 3b, 하네스 VOL_TELL_ACC=0.8 재현).
//   진짜의 20%는 조용히(quietReal), 헤드페이크의 20%는 시끄럽게(loudHeadfake) 뒤집어
//   "오르는데 거래량 얇으면 헤드페이크"가 확실한 규칙이 아니라 ~80% 경향이 되게 한다.
//   → 거래량만 보는 판단을 20% 배신해 톤+시세+거래량 종합(다층 판단)을 강제한다.
//   시장 경로와 분리된 전용 rng로 호출해 가격은 불변, 거래량 세기만 조정한다(batchService).
export function pickVolumeTells(targets: StockNewsTargets, rng: Rng): VolumeTells {
  const quietReals = new Set<string>();
  for (const code of targets.reals) {
    if (rng() >= STOCK_EARLY_VOL_TELL_ACC) quietReals.add(code); // 20% 조용
  }
  const loudHeadfakes = new Set<string>();
  for (const code of targets.headfakes) {
    if (rng() >= STOCK_EARLY_VOL_TELL_ACC) loudHeadfakes.add(code); // 20% 시끄러움
  }
  return { quietReals, loudHeadfakes };
}
