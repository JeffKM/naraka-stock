...
- `apply_daily_batch(...)` — 익일 전체 틱 + 뉴스 + 공시 생성. 마이그: `20260712010000_daily_batch`, `20260712030000_news_batch`, `20260717050000_news_source_in_batch`.
...
1. **틱 수 정합**: 장 시간(현재 12:00~24:00)에서 파생된 틱 수(144틱, 5분 간격)가 정확히 생성되는가. `TICKS_PER_DAY` 계산과 일치하는가.
...
2. **pg_net 5초 타임아웃**: 배치 내 HTTP(공시·익일뉴스)가 `net._http_response`에서 timed_out 나지 않도록 `timeout_milliseconds`가 60000인가. 공시·익일뉴스 누락 시 이 로그부터 확인.
...
# psql: 종목당 틱 수가 144인지, 전 종목 조회가 1000행에 잘리지 않는지
```
